...
void process_file(double array_to_populate[], FILE* pointer_to_data_file)
{
  ...
  while (fgets(line_buffer, LINESIZE, pointer_to_data_file)) {
      ...
      values_per_line = sscanf(line_buffer, "%d %d %d %d %d %d %d", &extracted_values[0], &extracted_values[1], &extracted_values[2], &extracted_values[3], &extracted_values[4], &extracted_values[5], &extracted_values[6]);// (for GradeScope submission)
 ...
 stored in the cell to the end of the array we are popuolating with data value. */
      for (i = 0; i < values_per_line; i++) {
          array_to_populate[i + readings_processed] = (double)extracted_values[i];
      }
